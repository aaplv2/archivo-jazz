...
export class SpotifyService {
  ...
  async revokeToken() {
    try {
      const token = this.getToken();
      if (!token) return { success: true, message: "No token to revoke" };

      // Spotify doesn't have a direct revoke endpoint, but we can invalidate by logging out
      this.logout();
  ...
  async regenerateToken() {
    try {
      // First revoke the current token
      ...
      if (loginUrl) {
        ...
        return {
          success: true,
          message: "Redirecting to Spotify for new authorization",
        };
      } else {
  ...
  clearAllStoredData() {
    ...
    const keysToRemove = [
      this.tokenKey,
      `${this.tokenKey}_expiration`,
      `${this.tokenKey}_refresh`,
      this.stateKey,
      "code_verifier",
      "spotify_player_initialized",
      "spotify_last_error",
    ];
    ...
    console.log("All Spotify authentication data cleared");
  }
  ...
  isTokenStale() {
    ...
    const timeUntilExpiry = Number.parseInt(expirationTime) - Date.now();
    const thirtyMinutes = 30 * 60 * 1000;

    // Consider stale if less than 30 minutes remaining
